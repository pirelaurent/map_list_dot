...
void main(){
 ...
 print(j1.runtimeType); // json _InternalLinkedHashMap<String, dynamic>
  ...
  print('----------------------------');
   ...
   var direct1 = { "name":"toto", "color" : "blue"  }; // _InternalLinkedHashMap<String, String>
   depiote(direct1);
   // cannot add age:20 to direct1 as <String,String>
   var direct2 = { "name":"toto", "color" : "blue", "age" :20  }; //Map : _InternalLinkedHashMap<String, Object>
   depiote(direct2);
   direct2 = { "name":"toto", "color" : "blue", "age" :20 , "scores": [11,12,13] }; //List : List<int>
   depiote(direct2);
   // direct2["scores"].add("pouet"); the method add is not defined for Object
 //----------------------- try the same with dynamic
  var direct3 = { "name":"toto", "color" : "blue", "age" :20  }; //Map : _InternalLinkedHashMap<String, Object>
  depiote(direct3);
  direct3 = { "name":"toto", "color" : "blue", "age" :20 , "scores": [11,12,13] }; //List : List<int>
  depiote(direct3);
  ...
  print("------------------------- print all in json --------------------------------");

  var json1 = json.decode(json.encode( { "name":"toto", "color" : "blue"  }));// Map : _InternalLinkedHashMap<String, dynamic>
  depiote (json1);
  var json2 = json.decode(json.encode({ "name":"toto", "color" : "blue", "age" :20  })); // Map : _InternalLinkedHashMap<String, dynamic>
  depiote(json2);

  var json3 = json.decode(json.encode({ "name":"toto", "color" : "blue", "age" :20 , "scores": [11,12,13] })); //List : List<dynamic>
  depiote(json3);
  ...
  print('------------------ try to recast ---------------');
  var cast1 =  { "name":"toto", "color" : "blue", "age" :20 , "scores": [11,12,13] };
  depiote(cast1); // _InternalLinkedHashMap<String, Object> // List<int>
  print(cast1["scores"].runtimeType);
  Map<String, dynamic >cast2 = cast1.cast<String,dynamic>(); //CastMap<String, Object, String, dynamic>
  depiote(cast2);
  List lili = cast1["scores"];
  List<dynamic> lili2 = lili.cast<dynamic>(); //CastList<int, dynamic> ?? à double entrée  ??
...
// et ça conserve les contraintes d'origine sur la partie int
  //  lili2.add("{}");
 print('---------------------------- test with normaliseByJson --------------------');
  var norm1 = normaliseByJson({ "name":"toto", "color" : "blue"  }); // Map :  _InternalLinkedHashMap<String, dynamic>
  depiote(norm1);
  var norm2 = normaliseByJson({ "name":"toto", "color" : "blue", "age" :20  });
  depiote(norm2);
  norm2 = normaliseByJson({ "name":"toto", "color" : "blue", "age" :20 , "scores": [11,12,13] }); //List : List<int>
  depiote(norm2);
